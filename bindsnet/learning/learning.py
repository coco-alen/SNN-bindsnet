...
class LTP(LearningRule):
    ...
    def _connection_update(self, **kwargs) -> None:
        ...
        if self.nu:
            import math
            rate = self.nu * math.exp(-self.A * )
            update = self.reduction(torch.bmm(source_s, target_x), dim=0)
            self.connection.w += rate * update
    ...
    def _conv2d_connection_update(self, **kwargs) -> None:
        ...
        if self.nu:
            ...
            self.connection.w += rate * post.view(self.connection.w.size())
...
class LTD(LearningRule):
    ...
    def _connection_update(self, **kwargs) -> None:
        ...
        if self.nu:
            import math
            rate = self.nu * K() * math.exp(-self.A * )
            rate += rate_pass 
            rate_pass = rate
            update = self.reduction(torch.bmm(source_s, target_x), dim=0)
            self.connection.w -= rate * update
